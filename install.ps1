...
$UseCLI = Enter-Host "Would you like to install/use the GitHub CLI?" -Loop

if ($UseCLI) {
    Write-Log "Installing GitHub CLI." -Type Info
    scoop install gh
    Write-Log "Github CLI installed." -Type Info
}
...
$clone = Enter-Host "Would you like to clone the project onto your computer?" -Loop

if ($clone) {
    $target = Read-Host "Where would you like to clone it? (default: here)"
    echo ($target -eq "")
}
